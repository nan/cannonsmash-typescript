...
export class Player {
    ...
    public canHitBall(ball: Ball): boolean {
        ...
        if (this.side === 1) { // Human's turn?
            return status === BallStatus.RALLY_TO_HUMAN ||
                   status === BallStatus.IN_PLAY_TO_HUMAN ||
                   status === BallStatus.SERVE_TO_HUMAN;
        } else { // AI's turn? (side === -1)
            return status === BallStatus.RALLY_TO_AI ||
                   status === BallStatus.IN_PLAY_TO_AI ||
                   status === BallStatus.SERVE_TO_AI;
        }
    }
